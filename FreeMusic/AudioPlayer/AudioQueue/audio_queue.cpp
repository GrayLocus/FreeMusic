...
void Audio_Queue::handleAudioPackets(UInt32 inNumberBytes, UInt32 inNumberPackets, const void *inInputData, AudioStreamPacketDescription *inPacketDescriptions)
{
    ...
    AQ_TRACE("%s: got data.  bytes: %lu  packets: %lu\n", __PRETTY_FUNCTION__, inNumberBytes, inNumberPackets);
    ...
    if (!inPacketDescriptions) {
        ...
        UInt32 base = inNumberBytes / inNumberPackets; // for CBR data, the packet size is a constant
    ...
    AQ_TRACE("**************no audio queue buffer to store audio packets, cache the data************");
