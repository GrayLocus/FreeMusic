...
-(MKNetworkOperation *)getSingerSongListWith:(long long)tinguid
                                            :(int)number
                                ...
                                errorHandler:(MKNKErrorBlock)errorHandler
{
    ...
    self.showError = YES;
    NSString *urlPath = [NSString stringWithFormat:@"/v1/restserver/ting?from=iphone&version=4.0&method=baidu.ting.artist.getSongList&format=json&order=2&tinguid=%lld&offset=0&limits=%d",tinguid,number];
    MKNetworkOperation *op = [self operationWithPath:urlPath];
    ...
    [op addCompletionHandler:^(MKNetworkOperation *completedOperation) {
        ...
        @try {
            NSDictionary * dict = respJson;
            if ([[dict allKeys] count] > 0) {
                NSLog(@"%@ : dict is not empty", self.class);
                NSArray * temp = [dict objectForKey:@"songlist"];
        ...
        @finally {
            ...
            NSLog(@"%@ : operation is handled", self.class);
        }
        ...
        @try{
            ...
            if ([[dictionary allKeys] count]>1) {
                ...
                for (NSDictionary * sub in songList) {
                    ...
                    song.size = [[sub objectForKey:@"size"] longLongValue];
                }
            }else{
                int errorcode = [[dictionary objectForKey:error_code] intValue];
                NSLog(@"%d",errorcode);
            }
            NSLog(@"before call completion block...");
            NSLog(@"song size %lld", song.size);
            completion(song);
        }
        @catch (NSException *exception) {
            KHError *appError = [[KHError alloc] initWithDomain:kHAppErrorDomain code:100 userInfo:nil];
...
//            NSLog(@"%@",sub);
            ...
            model.describeString = [sub objectForKey:@"description"];
