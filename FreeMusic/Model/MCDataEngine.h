...
-(MKNetworkOperation *)getSongLrcWith:(long long)tingUid
                                     :(long long)songID
                                     :(NSString *)lrclink
                WithCompletionHandler:(FMSongLrcResponseBlock)completion
                         errorHandler:(MKNKErrorBlock)errorHandler;

-(MKNetworkOperation *)downLoadSongWith:(long long)tingUid
                                       :(long long)songID
                                       :(NSString *)songLink
                WithCompletionHandler:(FMSongLinkDownLoadResponseBlock)completion
                         errorHandler:(MKNKErrorBlock)errorHandler;

-(MKNetworkOperation *)getTDMovieListWith:(int)pageNo
                                         :(int)pageSize
                                         :(int)channelId
                                         :(NSString *)sort
...
-(MKNetworkOperation *)searchTDMovieWith:(int)pageNo
                                        :(int)pageSize
                                        :(NSString *)orderBy
                                        :(NSString *)keyWord
                   WithCompletionHandler:(FMTDMovieListResponseBlock)completion
