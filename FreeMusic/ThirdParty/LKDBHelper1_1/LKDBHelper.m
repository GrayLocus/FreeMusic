...
if(model == nil) { \
    LKLog(@"model is nil");return NO; \
} \
checkClassIsInvalid(model.class)
...
-(NSMutableArray *)searchBase:(Class)modelClass where:(id)where orderBy:(NSString *)orderBy offset:(int)offset count:(int)count
{
    ...
    [self sqlString:query addOrder:orderBy offset:offset count:count];
...
-(void)sqlString:(NSMutableString*)sql addOrder:(NSString*)orderby offset:(int)offset count:(int)count
{
